...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    Out = np.dot(x, Wx) + b + np.dot(prev_h, Wh)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T)[::-1]:
        dh_i = dh[:, t, :] + dh0
        dx_i, dh0, dWx_i, dWh_i, db_i = rnn_step_backward(dh_i, cache[t])
        dx[:, t, :] = dx_i
        dWx += dWx_i
        dWh += dWh_i
        db += db_i 
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        if i == 0:
            h[:,i,:], next_c, cache[i] = lstm_step_forward(x[:,i,:], h0, prev_c, Wx, Wh, b)
        else:
            h[:,i,:], next_c, cache[i] = lstm_step_forward(x[:,i,:], h[:,i-1,:], prev_c, Wx, Wh, b)
        prev_c = next_c
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_backward(dh, cache):
    ...
    dWh = np.zeros((H,4*H))
    db = np.zeros((4*H,))
    dprev_c_i = np.zeros_like(dh0)
    for i in reversed(range(T)):
        dh_i = dh[:,i,:] + dh0
        dc_i = dprev_c_i
        dx_i, dprev_h_i, dprev_c_i, dWx_i, dWh_i, db_i = lstm_step_backward(dh_i, dc_i, cache[i])
        dx[:,i,:] += dx_i
        dh0 = dprev_h_i
        dWx += dWx_i
        dWh += dWh_i
